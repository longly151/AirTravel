...
class ChatListScreen extends Component<any, State> {
  ...
  getUsers = async () => {
    ...
    try {
      // Get sender id
      const senderResponse = await Api.get(`http://localhost:3000/users?filter={"phone": "${loginSelectorData.data.phone}"}`);
      this.sender_id = senderResponse.data[0]?._id;

      // Get users
      const results = await Api.get('http://localhost:3000/users');
